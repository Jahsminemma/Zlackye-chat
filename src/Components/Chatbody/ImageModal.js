import React from 'react';
import './ImageModal.css'


const ImageModal = ({ open, readerResult, Image,
    setImage, setOpen, pdf, setPdf, audio, setAudio }) => {
    const handleClose = () => {
        setOpen(false)
        setImage(false)
        setPdf(false)
        setAudio(false)
    }
    return (
        <div>
            {open ? (
                <div className="reviewed__image">
                    <span onClick={handleClose} className="close">&times;</span>
                    {Image ? <>
                        <img src={readerResult} alt="reviewed image" />
                        <p>{Image.name}</p>
                    </> : (" ")}
                    {pdf ? (
                        <>
                            <img src="data:image/png;base64," alt="reviewed image" />
                            <p>{pdf.name}</p>
                        </>
                    ) : null}
                    {audio ? <>
                        <img src="https://image.shutterstock.com/image-vector/headphones-neon-sign-luminous-signboard-260nw-1686845326.jpg" alt="reviewed image" />
                        <p>{audio.name}</p>
                    </> : (" ")}

                </div>
            ) : null}
        </div>
    )
}
export default ImageModal